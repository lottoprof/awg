...
## Назначение
Документ фиксирует фактический прогон установки на сервере `awgesrv`.
...
## Чек-лист
- [x] Подключение по SSH под пользователем `awgesrv`
...
- [x] Настройка `EXPORT_DIR=/home/awgesrv/export`
- [x] Проверка выгрузки `.conf` и `.png` в `/home/awgesrv/export`
...
- После reboot сессия `awgsetup` отсутствовала.
  Решение: использовать фактически существующую сессию `tmux attach -t awgesrv`.
...
- Первые экспортные файлы в `/home/awgesrv/export` создавались с владельцем `root:root`, поэтому `scp` под `awgesrv` был неудобен.
  Решение: добавить `EXPORT_DIR` в `/etc/amnezia/params`, обновить `awg-add-client` и выставлять владельца exported-файлов по владельцу каталога `/home/awgesrv/export`.
