...
## Решение по IPv6
Для первой версии принимаем модель `IPv4-only`.
...
Перед настройкой правил нужно точно определить внешний интерфейс сервера.

Перед этим еще раз убедитесь, что вход по новому SSH-порту стабильно работает.
...
На сервере должен быть доступен `iptables`.
...
Так как в первой версии IPv6 не используем, отключаем его на сервере через `sysctl`.
...
Для маршрутизации трафика клиентов через сервер нужно включить `ip_forward`.
...
Чтобы правила переживали перезагрузку, ставим `iptables-persistent`.
...
Перед применением новой схемы очищаем старое состояние.
...
Сначала открываем то, без чего сервер потеряет управляемость.
...
Опционально для диагностики можно разрешить `ping`:
```bash
sudo iptables -A INPUT -p icmp --icmp-type echo-request -j ACCEPT
```

## Шаг 8. Настройка FORWARD для AWG
Разрешаем клиентам `AWG` выходить наружу и принимать обратный трафик.
...
## Шаг 9. Настройка NAT
Включаем маскарадинг на внешнем интерфейсе сервера.
...
После явных разрешающих правил включаем политики по умолчанию.
...
До сохранения убедитесь, что правила выглядят именно так, как ожидается.
...
Сохраняем текущие правила для загрузки после перезапуска сервера.
...
После включения firewall обязательно убедитесь, что SSH не отрезан.
...
После сохранения правил нужно убедиться, что они вернутся после reboot.
...
## Следующий этап
После этого этапа переходим к установке `AmneziaWG`:
- подготовка зависимостей;
- при необходимости обновление ядра и заголовков;
- установка `AWG`;
- проверка наличия рабочих бинарников и systemd unit.
