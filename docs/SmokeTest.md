...
Убедитесь, что `awg0` поднимается и systemd видит сервис.
...
На клиентском устройстве включите туннель и проверьте внешний IP.
...
- добавить dry-run и verify-режимы в `awg-*` скрипты;
- оформить документ по эксплуатации day-2;
- затем переходить к каркасу shell-проекта для полного deployment flow.
