...
Важно:
- для `v1` этот этап выполнять как можно раньше, еще до изменения `sshd` и до полного bootstrap;
...
Сначала зафиксировать версию ядра, под которую будут нужны заголовки.
...
Правило:
- если после обновления появился новый kernel package, reboot делать сразу, а не переносить его на поздний этап.
...
- если есть сомнение по совместимости, ядро нормализовать сразу;
...
- включить source repositories вручную в файлах `APT`;
- затем выполнить:
```bash
sudo apt update
```
...
Проверить, что пакетная установка завершилась без скрытых проблем.
